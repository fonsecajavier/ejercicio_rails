...
describe EmployeesController do
  ...
  context "acting based on non-existing records" do
    before(:each) do
      ...
      # another way in case that you prefer not to use factory's associations:
    ...
    context "POST create" do
      it "should render :new once again on invalid POST params" do
        @employee.first_name = ""
        post :create, :account_id => @employee.account, :employee => @employee.attributes
        response.should render_template("new")
      end

      it "should render employees_path (index of employees) on valid creation" do
        post :create, :account_id => @employee.account, :employee => @employee.attributes
        response.should redirect_to(account_path(@employee.account))
      end
  ...
  context "acting based on existing records" do
    ...
    context "GET edit" do
      it "assigns correct employee to the template depending on its id" do
        get :edit, {:account_id => @employee.account, :id => @employee}
    ...
    context "PUT update" do
      it "should render :edit once again on invalid PUT params" do
        put :update, {:account_id => @employee.account, :id => @employee,
                       :employee => {:first_name => ""}}
    ...
    context "DELETE destroy" do
      it "should render account_path (index of employees) on valid destruction" do
        delete :destroy, {:account_id => @employee.account, :id => @employee}
        response.should redirect_to(account_path(@employee.account))
      end
